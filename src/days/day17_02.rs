...
type Cubes = Vec<Vec<Vec<Vec<Cube>>>>;
...
impl PDim {
    fn cube_at(&self, pos: &Pos) -> &Cube {
        return &self.cubes[pos.w][pos.z][pos.y][pos.x];
    }
...
pub fn day17_02() {
    ...
    loop {
        ...
        walk_pdim(&pdim, |cube, pos| {
            ...
            new_cubes[pos.w][pos.z][pos.y][pos.x] = Cube {
                state: new_state,
...
fn to_pos(x: usize, y: usize, z: usize, w: usize) -> Pos {
    return Pos {
        x: x,
        y: y,
        z: z,
...
fn init_neighbors(cubes: &mut Cubes) {
    for w in 0..PDIM_SIZE {
        for z in 0..PDIM_SIZE {
            for y in 0..PDIM_SIZE {
                for x in 0..PDIM_SIZE {
                    cubes[w][z][y][x].neighbors =
                        get_neighbors(x as isize, y as isize, z as isize, w as isize);
                }
            }
        }
    }
...
fn get_neighbors(x: isize, y: isize, z: isize, w: isize) -> Vec<Pos> {
            ...
            for nz in (z - 1)..=(z + 1) {
                for nw in (w - 1)..=(w + 1) {
                    if !(nx == x && ny == y && nz == z && nw == w)
                        ...
                        && (0..PDIM_SIZE).contains(&(nw as usize))
                    {
                        neighbors.push(to_pos(nx as usize, ny as usize, nz as usize, nw as usize));
                    }
                }
            }
        }
